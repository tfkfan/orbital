...
public class GameManager<
    ...
    private final PlayerFactory<P, GR> playerFactory;
    private final GameMapFactory<GM> gameMapFactory;
    private final GameRoomFactory<GM, GR> gameRoomFactory;

    public GameManager(String verticleId,
                       Vertx vertx,
                       RoomProperties roomProperties,
                       PlayerFactory<P, GR> playerFactory,
                       GameMapFactory<GM> gameMapFactory,
                       GameRoomFactory<GM, GR> gameRoomFactory) {
    ...
    protected void onMessage(Message<JsonObject> message) {
        ...
        if (json.containsKey(Fields.action)) {
            ...
            if (actionType.equals(ActionType.CREATE)) {
                final GM gameMap = gameMapFactory.createMap();
                final UUID roomId = UUID.fromString(json.getString(Fields.roomId));
                final JsonArray sessions = json.getJsonArray(Fields.sessions);
                final GR room = gameRoomFactory.createGameRoom(verticleId, roomId, gameMap, roomProperties);
